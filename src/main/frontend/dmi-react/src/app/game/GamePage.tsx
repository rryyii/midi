                ...
                {data.content.map((game: Game) => {
                    return (
                        <Link to={`/games/${game.id}`} key={`${game.id}-page`}>
                            {game.cover != null ? <img src={`https://images.igdb.com/igdb/image/upload/t_cover_big/${game.cover.image_id}.jpg`} loading="lazy" alt={`cover image of ${game.name}`} className={`gameCard`}/> :
                                <h4>{game.name}</h4>}
                        </Link>
                    );
                })}
